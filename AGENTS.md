...
- Supabase Auth owns OAuth client credentials and refresh tokens; the database stores only public `client_id`, enabled/revoked state, capability grants, and audit history. Direct MCP hosts use separate public manual clients per host/environment while Dynamic Client Registration remains disabled; environment UUIDs are provisioned only through the service façade and never migrations. The official Production CLI client keeps the exact capability class `CLI-RPC-01`, `DB-CORE-READ-01`, `DB-CORE-WRITE-01`, `NX-CORE-02`, and `EDGE-BUNDLE-01`; changing that environment-specific grant is a database-engine-owned operator action through `api.svc_oauth_client_configure`, guarded by exact before/after readback and durable audit evidence. First-party sessions without `client_id` retain existing `auth.uid()` behavior. OAuth relation reads require `DB-CORE-READ-01`; raw table DML remains ACL-closed, actor create/save/delete uses the existing command RPCs under `DB-CORE-WRITE-01`, and the two exact LifecycleModel bundle commands use `EDGE-BUNDLE-01` rather than the broad `CLI-RPC-01`; every relation/RPC route fails closed through restrictive RLS plus the manifest-backed PostgREST pre-request hook
