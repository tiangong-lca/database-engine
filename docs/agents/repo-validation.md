...
lastReviewedNote: "Reviewed for Issues #551/#552: 289-migration reset, forced-RLS CAS indexability, 10,000-row duplicate pressure, unchanged Search budgets, and the unified CLI restart/Preview gates."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Portal catalog-summary executable examples | blank reset plus `supabase/tests/20260825_portal_public_catalog_v1.sql` and `20260826_portal_candidate_first_search.sql`; run the summary cardinality benchmark with 20 samples | after Dev/Main deployment, resolve examples dynamically through the anonymous summary and execute each through its declared public Search RPC; capture a sanitized forced-RLS plan and log only query kind/dataset kind/status/latency/count | Classification examples require a trimmed public code of at least four characters and prefer Process. CAS selection uses one CAS-index GroupAggregate, keeps at most 64 values that occur exactly once across retained projection history, and rechecks the chosen row against the shared latest CTE; Search itself remains complete. The projection's row-neutral Portal policy is valid only with forced RLS, the exact validated state-100/200 CHECK, and the live assertion binding both. Under that role, CAS equality must be in the exact index's `Index Cond`, not a JSON filter. Ordinary lexical/UUID/invalid-CAS text retains PGroonga. Summary p95 stays <=250 ms even with 10,000 duplicate CAS rows; every emitted classification/CAS example is nonempty, p95 <2 seconds, every sample <8 seconds. Do not raise either timeout, log the query, or infer a substitute. |
