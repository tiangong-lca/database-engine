...
lastReviewedNote: "Reviewed for Issue #582: direct OAuth proof remains exact; workflow proof now accepts official Preview skip only for a verified docs-only diff and fails closed for every supabase/ change."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| `.github/workflows/supabase-dev.yml` | inspect and parse YAML; run `python scripts/test_resolve_migration_head.py` and `python scripts/test_supabase_dev_workflow_contract.py`; prove the Preview job checks out the exact PR head, validates exact base/head commits, and emits `required=false` only when `git diff --quiet <base> <head> -- supabase/` succeeds; retain the existing persistent-Dev one-push/one-PATCH contract and Preview one-PATCH/no-deployment contract | on a docs-only same-repository PR, prove the official Supabase App `skipped` check does not fail the repository job and no Preview authority or hosted mutation step runs. On an exact PR head with any `supabase/` change, require all authority inputs, one successful official `Supabase Preview` check, exact disposable BranchResponse identity, PostgREST PATCH/readback, enabled public-key selection, and anonymous Hybrid/sitemap probes | Fork PRs skip before authority. Exact zero-diff is the only accepted no-Preview outcome; missing commits or diff errors fail. Every Supabase-changing PR retains the existing fail-closed App identity, branch equality, main/Dev exclusion, credential isolation, two exact Management API PATCHes across jobs, and no Functions/config/schema deployment from the Preview runtime job. |
