...
do $bound$ begin
  if (select count(*) from (select 1 from public.processes source where source.id >= 'f0000000-0000-0000-0000-000000000000'::uuid and source.state_code in (100,200) and source.modified_at is not null and jsonb_typeof(source.json) = 'object' and jsonb_typeof(source.json -> 'processDataSet') = 'object' limit 20001) bounded) > 20000 then
...
with source_rows as materialized (
  ...
  from public.processes source where source.id >= 'f0000000-0000-0000-0000-000000000000'::uuid and source.state_code in (100,200) and source.modified_at is not null and jsonb_typeof(source.json) = 'object' and jsonb_typeof(source.json -> 'processDataSet') = 'object'
...
-- this page, and the dual writer subsequently installs the committed newer value.
do $bound$ begin
  if (select count(*) from (select 1 from public.flows source where source.id >= 'f0000000-0000-0000-0000-000000000000'::uuid and source.state_code in (100,200) and source.modified_at is not null and jsonb_typeof(source.json) = 'object' and jsonb_typeof(source.json -> 'flowDataSet') = 'object' limit 20001) bounded) > 20000 then
    raise exception 'Portal names shard exceeds reviewed 20000-row bound; use a reviewed finer partition';
  end if;
end $bound$;
with source_rows as materialized (
  select source.id, source.version, source.state_code, source.modified_at, source.json
  from public.flows source where source.id >= 'f0000000-0000-0000-0000-000000000000'::uuid and source.state_code in (100,200) and source.modified_at is not null and jsonb_typeof(source.json) = 'object' and jsonb_typeof(source.json -> 'flowDataSet') = 'object'
  order by source.id, source.version for share of source
), payloads as materialized (
...
select 15, count(*) filter(where dataset_kind='process'), count(*) filter(where dataset_kind='flow')
from private.portal_catalog_search_rows_v2 where id >= 'f0000000-0000-0000-0000-000000000000'::uuid
