...
lastReviewedNote: "Reviewed for Issue #582: direct OAuth clients remain exact and DCR-disabled; Preview runtime proof now runs only for a base-to-head supabase/ diff, while docs-only official skips succeed."
related:
  - .docpact/config.yaml
...
- PR Preview runtime verification belongs to the pull-request-only job in `.github/workflows/supabase-dev.yml`: forks skip before authority is available; same-repository PRs first check out the exact head and compare the exact base/head commits under `supabase/`. A zero-diff PR succeeds without Preview authority or hosted mutation and may accept the official Supabase App `skipped` result. Any `supabase/` change fails closed unless the access token, main-parent ref, and persistent-Dev ref all exist; it then waits for exactly one successful `Supabase Preview` check on the PR head from official Supabase App id `330661`, slug/owner `supabase`, captures the 20-character expected ref from that check's exact dashboard URL, and independently resolves exactly one non-default, non-persistent BranchResponse from pinned CLI `branches list --output json` by Git branch, PR number, and parent ref. The two refs must match and differ from both main and Dev before one exact three-field PostgREST PATCH/readback. A separate key step reads the raw no-reveal Management API response, accepts only an explicitly enabled, shape-valid publishable or legacy anon key, masks and exports that public key, then clears the PAT and raw JSON. The subsequent Portal Hybrid and sitemap readiness probes each have one 300-second wall-clock deadline and contain no PAT, `Authorization`, `Cookie`, or service credential; once the public key is available, sitemap verification still runs independently when Hybrid fails. The job never links, pushes migrations, deploys Functions, or targets persistent Dev or production.
