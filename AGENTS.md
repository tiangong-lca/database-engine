...
lastReviewedNote: "Reviewed for Issue #566: OAuth client authorization is database-owned through a revocation registry, restrictive core-table RLS, and the manifest-backed PostgREST pre-request gate."
related:
  - .docpact/config.yaml
...
- Supabase Auth owns OAuth client credentials and refresh tokens; the database stores only public `client_id`, enabled/revoked state, capability grants, and audit history. First-party sessions without `client_id` retain existing `auth.uid()` behavior, while OAuth relation and RPC access fails closed through restrictive RLS plus the manifest-backed PostgREST pre-request hook
