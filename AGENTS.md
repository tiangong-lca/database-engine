...
lastReviewedCommit: 1c8334c5fdef42ec47ce1e5956ff8afd045eb515
...
- Supabase Auth owns OAuth client credentials and refresh tokens; the database stores only public `client_id`, enabled/revoked state, capability grants, and audit history. First-party sessions without `client_id` retain existing `auth.uid()` behavior. OAuth relation reads require `DB-CORE-READ-01`; raw table DML remains ACL-closed, actor create/save/delete uses the existing command RPCs under `DB-CORE-WRITE-01`, and the two exact LifecycleModel bundle commands use `EDGE-BUNDLE-01` rather than the broad `CLI-RPC-01`; every relation/RPC route fails closed through restrictive RLS plus the manifest-backed PostgREST pre-request hook
