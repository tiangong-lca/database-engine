...
class DatabaseContractManifestTest(unittest.TestCase):
    ...
    def test_checked_manifest_has_unique_classification_and_92_canonical_files(self) -> None:
        ...
        self.assertEqual(len(classified["canonical-pgtap"]), 92)
    ...
    def test_suite_evidence_binds_commit_head_cli_manifest_and_file_list(self) -> None:
        ...
        self.assertEqual(evidence["migrationHead"], "20260804123000")
    ...
    def test_all_106_sql_assets_are_owned_once_but_only_92_are_canonical_pgtap(self) -> None:
        classified = runner.validate_manifest(self.manifest, self.tracked)
        ...
        self.assertEqual(len(sql_paths), 106)
    ...
    def test_issue_390_runtime_is_mandatory_exact_canonical_local_wiring(self) -> None:
        ...
        script = runner.RESULT_API_FACADE_RUNTIME_SCRIPT
        self.assertEqual(
            runner.result_api_facade_runtime_scripts([contract], skip_data_api=False),
            [script],
        )
        self.assertEqual(
            runner.result_api_facade_runtime_scripts([], skip_data_api=False), []
        )
        self.assertEqual(
            runner.result_api_facade_runtime_scripts([contract], skip_data_api=True), []
        )
