...
lastReviewedNote: "Reviewed through Issues #339/#341, #346, and #351: retain ACL convergence, opaque-key Worker probe qualification, and million-row qualification with new-only evidence, URL redaction, zero-WAL retry, and persistent-dev rollback/readback proof."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| global schema boundary or populated-upgrade qualification | on an isolated local stack and clean exact commit, run `python scripts/test_production_equivalent_upgrade.py --evidence-out <new-absolute-path-outside-worktree>` with the contract's one-million-row default; run `python -m unittest scripts/test_production_equivalent_upgrade_unit.py` for offline output safety | inspect the emitted redacted mode-0600 manifest; compare fixture/upgrade seconds and migration WAL to the checked budgets; retain exact commit, migration SHA-256, base/head history, row/PK/content oracle hashes, per-migration fault results, lock-timeout recovery, compatible-reader proof, role matrix, and catalog-invariant hash in the PR | The harness is destructive only to the selected local stack and must never use `--linked` or a hosted URL. Both PostgreSQL URL schemes are redacted from command and exception output. Evidence uses exclusive/no-follow creation, refuses existing targets or symlinks, and is fsynced before success. Its fixture spans identity, review, notification, audit, active/leased/retry/failed Worker jobs, artifacts, package/cache, release, and closure. Every pending migration must roll back cleanly under injected failure; an incompatible lock must fail within five seconds; the real roll-forward and no-op retry must preserve all physical-table rows, constraints, ACL/RLS, policies, triggers, and publication membership, and the no-op retry must emit exactly zero WAL bytes. Update the base/head contract deliberately when migration scope advances. |
...
| public object inventory/dependency closure | `supabase db reset`; run `python -m unittest scripts/test_public_inventory_closure.py`, `supabase test db supabase/tests/20260801_public_inventory_dependency_closure.sql --local`, and `python scripts/public_inventory_closure.py --check` | refresh exact-SHA consumer evidence with `--scan-consumers <workspace-root>` only when a pinned consumer commit changes; review every emitted dynamic-SQL and owner/runtime residue entry before Contract | The gate covers tables, views, materialized views, functions/procedures, identity arguments, ACL/RLS/default privileges, FK/rewrite/trigger/policy/composite/body dependencies, migration order, and consumer evidence. Unknown consumers remain explicit blockers and never imply retirement. |
