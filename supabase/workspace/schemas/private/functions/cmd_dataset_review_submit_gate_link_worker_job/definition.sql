CREATE OR REPLACE FUNCTION "private"."cmd_dataset_review_submit_gate_link_worker_job"("p_gate_run_id" "uuid", "p_action" "text" DEFAULT 'ensure'::"text") RETURNS "jsonb"
...
begin
  ...
  if v_run.worker_job_id is null and v_run.status in ('queued', 'running') then
    v_worker_result := private.cmd_dataset_review_submit_gate_enqueue_worker_job(
      v_run.dataset_table,
      v_run.dataset_id,
...
ALTER FUNCTION "private"."cmd_dataset_review_submit_gate_link_worker_job"("p_gate_run_id" "uuid", "p_action" "text") OWNER TO "postgres";

REVOKE ALL ON FUNCTION "private"."cmd_dataset_review_submit_gate_link_worker_job"("p_gate_run_id" "uuid", "p_action" "text") FROM PUBLIC;

GRANT ALL ON FUNCTION "private"."cmd_dataset_review_submit_gate_link_worker_job"("p_gate_run_id" "uuid", "p_action" "text") TO "service_role";

GRANT ALL ON FUNCTION "private"."cmd_dataset_review_submit_gate_link_worker_job"("p_gate_run_id" "uuid", "p_action" "text") TO "api_internal_executor";
