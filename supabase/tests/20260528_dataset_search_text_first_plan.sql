...
select ok(
  strpos(pg_get_functiondef('private.search_flows_latest_impl(text,jsonb,bigint,bigint,text,text,uuid,integer,text[])'::regprocedure), 'where f.extracted_md &@~| $14') > 0
  ...
  'flow latest search separates extracted_md scan from source/filter predicates'
);
...
select ok(
  strpos(pg_get_functiondef('private.search_processes_latest_impl(text,jsonb,bigint,bigint,text,text,uuid,integer,text,text[])'::regprocedure), 'where p.extracted_md &@~| $11') > 0
  ...
  'process latest search separates extracted_md scan from source/filter predicates'
);
...
select ok(
  strpos(pg_get_functiondef('private.search_lifecyclemodels_latest_impl(text,jsonb,bigint,bigint,text,text,uuid,integer,text[])'::regprocedure), 'where l.extracted_md &@~| $10') > 0
  ...
  'lifecyclemodel latest search separates extracted_md scan from source/filter predicates'
);
