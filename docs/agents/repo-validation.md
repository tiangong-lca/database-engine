...
| process/flow hybrid-search query shape, HNSW settings, executor role, or vector indexes | `supabase db reset`; run `supabase/tests/20260727_hybrid_search_staging_plan_governance.sql`, `supabase/tests/20260528_semantic_hybrid_candidate_helpers.sql`, `supabase/tests/20260528_hybrid_semantic_candidate_limit_hotfix.sql`, `supabase/tests/20260903_portal_flow_adaptive_semantic_v2.sql`, and `supabase/tests/20260904_portal_flow_semantic_executor_alignment.sql`; for version-aware Portal changes also run `supabase/tests/20260902_portal_version_search_v2.sql` | after the migration reaches persistent `dev`, run the exact read-only `supabase/tests/benchmarks/20260727_hybrid_search_staging_explain.sql` profile against ref `submidrhbtknjxfympna`; compare process, broad-flow, Product-flow, and full-RPC timings/buffers with Issue #292; for the adaptive Flow V2 path, compare cold and warm buffers/timings for zero, small, boundary, and broad geography/access-level candidate sets without changing the session's production-equivalent HNSW settings; rerun performance and security advisors | Use the benchmark's checked-in Edge route lexical parameter profile with its redacted real staging vectors; never synthesize lexical terms from the first Markdown tokens because shared headings create an artificial near-whole-table match. Do not use a seed-only Preview as production-cardinality proof, expose raw text/UUID/vector values, or run production `EXPLAIN ANALYZE`. Empty JSON filters must fold away under a custom plan. General and broad Flow paths retain strict-order iterative HNSW with one 10x semantic bound and a 200-row floor. Version-aware Portal Flow geography/access-level filters may route to exact distance evaluation only after a bounded 2,001-key facet probe proves at most 2,000 exact-version candidates; prove zero-match behavior, both sides of the cutoff, full-filter recheck, exact-version identity, deterministic ordering, and no duplicate vector index. Executor changes must prove the Flow leaf body/settings are byte-stable, `portal_public_executor` remains NOLOGIN/NOINHERIT/NOBYPASSRLS under the exact public-state policy, only `flows.embedding_ft` is newly granted, the internal caller remains admitted, and external roles remain denied. |
...
`20260902_portal_version_search_v2.sql`. When changing the adaptive Flow V2
semantic path, also run `20260903_portal_flow_adaptive_semantic_v2.sql`,
`20260904_portal_flow_semantic_executor_alignment.sql`, and the three adjacent
semantic/HNSW governance suites from the proof matrix. Prove
...
Existing V1 and Process V2 definitions and immutable projection/context/facet
...
forcing planner indexes. For the adaptive Flow V2 path, record cold and warm
...
`scan_mem_multiplier=2` settings. Capture the broad Flow plan as
`portal_public_executor`, verify it naturally names the source HNSW index, and
retain the exact public-state RLS policy; do not use BYPASSRLS or a globally
forced planner setting as proof. Real public-data relevance and deployed
