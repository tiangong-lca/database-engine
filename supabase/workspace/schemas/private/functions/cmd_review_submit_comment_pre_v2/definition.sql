CREATE OR REPLACE FUNCTION "private"."cmd_review_submit_comment_pre_v2"("p_review_id" "uuid", "p_json" "jsonb", "p_comment_state" integer DEFAULT 1, "p_audit" "jsonb" DEFAULT '{}'::"jsonb") RETURNS "jsonb"
...
begin
  ...
  if p_comment_state = 1 then
    for v_ref in
      select *
      from api.cmd_review_extract_refs(v_comment_json)
    loop
    ...
    loop
      execute format(
        ...
        using api.cmd_review_append_review_ref(v_target.reviews, p_review_id),
              v_target.dataset_id,
