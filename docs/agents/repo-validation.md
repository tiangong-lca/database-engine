...
lastReviewedNote: "Reviewed merged Issues #308 and #310: retain artifact-delivery upgrade proof and add local/Preview pgTAP, seven-index coverage, v2 RPC transport, strict owner-draft, query escaping, lint, and staged EXPLAIN evidence for extracted_md lexical search."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| `extracted_text` retirement or Hybrid/Search v2 lexical contract | `supabase db reset`; run `supabase/tests/20260613_hybrid_search_query_terms_escape.sql`, `supabase/tests/20260727_foundation_dataset_semantic_hybrid_search.sql`, and `supabase/tests/20260729_extracted_md_lexical_search_contract_v2.sql` plus the nearby semantic/HNSW regressions | on the exact Preview branch, smoke all seven v2 Hybrid RPCs and `search_processes_latest_v2`, including strict owner-only draft scope; after merge to persistent `dev`, use redacted real parameters for read-only `EXPLAIN (ANALYZE, BUFFERS)` and compare advisors before Contract cleanup | Expand must build all seven `extracted_md` PGroonga indexes before switching lexical execution, retain old signatures during consumer cutover, and expose one `lexical_weight`. Contract is a separate post-burn-in migration that drops legacy objects with `RESTRICT`; never combine production column/function deletion with the Expand PR. |
