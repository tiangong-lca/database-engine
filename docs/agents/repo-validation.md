...
lastReviewedNote: "Reviewed for Issue #566: OAuth client admission now requires a private revocation registry, capability grants, restrictive core-table RLS, a PostgREST pre-request RPC gate, and transport-level revocation proof."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| OAuth client registry, capability grants, or PostgREST pre-request authorization | `supabase db reset --no-seed`; run `supabase/tests/20260830_oauth_client_capability_foundation.sql`, `20260806_api_contract_closure.sql`, and `20260805_full_schema_cutover.sql`; restart local Supabase before transport proof so the configured `public,api,graphql_public` schema list is active | with a locally signed test JWT containing `client_id`, prove an admitted relation read returns 200, an ungranted zero-argument RPC returns SQLSTATE `42501`, and disabling the same client makes the previously admitted read return `42501`; repeat against exact Preview before persistent Dev | Supabase Auth owns OAuth client secrets and refresh tokens. Database state stores only public `client_id`, kind, enabled/revoked state, capability grants, and audit history. First-party sessions without `client_id` preserve existing `auth.uid()` behavior. Every authenticated public relation remains protected by a restrictive capability policy, every authenticated RPC route has one unambiguous manifest capability, unknown routes fail closed, and the service-only configuration facade atomically replaces grants. Do not hardcode environment-specific client IDs in migrations or grant custom database access from OAuth scopes. |
