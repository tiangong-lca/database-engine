...
declare
  ...
  v_run private.dataset_review_submit_gate_runs%rowtype;
  v_existing private.dataset_review_submit_gate_runs%rowtype;
  v_supersedes uuid;
  v_link_result jsonb;
begin
  ...
  v_dataset_row := api.cmd_review_get_dataset_row(p_table, p_id, p_version, false);
  ...
  if p_gate_run_id is not null then
    ...
    if v_action = 'read' then
      return jsonb_build_object(
        'ok', true,
        'data', private.cmd_dataset_review_submit_gate_payload(
          v_existing,
          case
    ...
    if v_action = 'ensure' and v_existing.revision_checksum = p_revision_checksum then
      return private.cmd_dataset_review_submit_gate_link_worker_job(v_existing.id, 'ensure');
    end if;
  ...
  if v_action in ('ensure', 'read') and p_gate_run_id is null then
    ...
    if v_run.id is not null then
      if v_action = 'ensure' then
        return private.cmd_dataset_review_submit_gate_link_worker_job(v_run.id, 'ensure');
      end if;

      return jsonb_build_object(
        'ok', true,
        'data', private.cmd_dataset_review_submit_gate_payload(v_run)
      );
    end if;
  ...
  v_link_result := private.cmd_dataset_review_submit_gate_link_worker_job(v_run.id, v_action);
...
ALTER FUNCTION "api"."cmd_dataset_review_submit_gate"("p_table" "text", "p_id" "uuid", "p_version" "text", "p_revision_checksum" "text", "p_policy_profile" "text", "p_report_schema_version" "text", "p_action" "text", "p_gate_run_id" "uuid", "p_audit" "jsonb") OWNER TO "postgres";

REVOKE ALL ON FUNCTION "api"."cmd_dataset_review_submit_gate"("p_table" "text", "p_id" "uuid", "p_version" "text", "p_revision_checksum" "text", "p_policy_profile" "text", "p_report_schema_version" "text", "p_action" "text", "p_gate_run_id" "uuid", "p_audit" "jsonb") FROM PUBLIC;

GRANT ALL ON FUNCTION "api"."cmd_dataset_review_submit_gate"("p_table" "text", "p_id" "uuid", "p_version" "text", "p_revision_checksum" "text", "p_policy_profile" "text", "p_report_schema_version" "text", "p_action" "text", "p_gate_run_id" "uuid", "p_audit" "jsonb") TO "api_internal_executor";

GRANT ALL ON FUNCTION "api"."cmd_dataset_review_submit_gate"("p_table" "text", "p_id" "uuid", "p_version" "text", "p_revision_checksum" "text", "p_policy_profile" "text", "p_report_schema_version" "text", "p_action" "text", "p_gate_run_id" "uuid", "p_audit" "jsonb") TO "authenticated";
