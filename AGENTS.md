...
lastReviewedNote: "Reviewed for Issues #412 and #323: the generic static-SQL SECURITY DEFINER rule is business-neutral, and latest-event review notifications remain database-owned."
related:
  - .docpact/config.yaml
...
- persistent `dev` deployment is serialized and must pass the fresh local canonical/freeze-activated database contract before `supabase db push`; deployment then runs `scripts/apply_postgrest_config.py --apply` followed by an independent `--check` readback; the script may PATCH only `db_schema`, `db_extra_search_path`, and `max_rows`, then must GET-readback the same exact project ref; a mismatched readback must restore and verify the prior allowlisted snapshot
...
- a non-core relation family with a historical browser-role grant must first use a checked `*_pre_ddl.v1.json` contract: while `ddlAuthorized=false`, committed migration history is append-only on every governed parent edge and target-neutral, proven non-executing static migrations may proceed; direct targets and their recursive application-object dependency closure stay protected; an additive service-only `api` facade needs an exact reviewed blob, explicit `pg_catalog` signature types, and fixed-version PostgreSQL AST semantic proof, while opaque/dynamic or data-rewriting execution, moves into exposed schemas, historical/service access removal, and non-service internal/protected grants remain hard denied until exact static, attributed runtime, and owner evidence authorize physical DDL
...
- public-boundary inventory changes must run offline `python scripts/public_inventory_exact_head.py --check`; Issue #405 binds exact `dev` SHA `c5356d2`, the SHA-256 of every migration blob through head `20260804090000`, the live database's exact applied-version set, the 397-object ledger, the exactly-once 388-residue partition, and a blocked identity-only Contract DROP checklist. Live regeneration and comparison require explicit loopback URLs; these commands never emit DDL and can never set `contractReady=true`. The immutable #338 baseline remains under `public_object_inventory.genesis.*` for historical lineage.
