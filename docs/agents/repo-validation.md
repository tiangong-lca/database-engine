...
| identity/collaboration boundary Expand | `supabase db reset`; `supabase test db --local supabase/tests/20260801_identity_collaboration_expand.sql`; `python -m unittest scripts/test_identity_collaboration_expand_static.py`; `python scripts/test_identity_collaboration_data_api.py`; `python scripts/test_identity_collaboration_concurrency.py`; rerun Issue #339 and #333 gates | explicitly opt into `python scripts/run_database_contract.py --suite canonical-local --skip-reset --run-destructive-identity-qualification` on one disposable loopback stack to rehearse owner/custom-ACL convergence, repeatable operator rollback/roll-forward, exact-state migration reapply, zero-target rollback, and five-second lock-failure atomicity; then run the global one-million-row populated upgrade | All Issue #355 probes bind and verify one `IDENTITY_COLLABORATION_DATABASE_URL`/`DATABASE_URL` against the selected `SUPABASE_WORKDIR` stack. All sixteen physical objects remain `public` in Expand. Reuse `api.review_comments_v1`; keep notifications browser access on reviewed RPCs; identity DTOs are service-only; other DTOs are authenticated/service-only. Preserve all eight audited public routine OIDs/definitions/properties/ACLs. Contract waits for exact-SHA consumer zero and hosted-dev burn-in. |
