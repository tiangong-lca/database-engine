CREATE OR REPLACE FUNCTION "private"."cmd_dataset_review_submit_gate_payload"("p_run" "private"."dataset_review_submit_gate_runs", "p_status_override" "text" DEFAULT NULL::"text") RETURNS "jsonb"
    LANGUAGE "sql" STABLE
    SET "search_path" TO 'private', 'api', 'public', 'util', 'extensions', 'pg_temp'
    AS $$
  with worker as (
    select w.*
    from private.worker_jobs as w
      ...
      'workerJob',
        case
          when shaped.worker_job_id is null then null
            ...
            select private.worker_job_payload(worker, false)
            from worker
          )
        end,
...
ALTER FUNCTION "private"."cmd_dataset_review_submit_gate_payload"("p_run" "private"."dataset_review_submit_gate_runs", "p_status_override" "text") OWNER TO "postgres";

REVOKE ALL ON FUNCTION "private"."cmd_dataset_review_submit_gate_payload"("p_run" "private"."dataset_review_submit_gate_runs", "p_status_override" "text") FROM PUBLIC;

GRANT ALL ON FUNCTION "private"."cmd_dataset_review_submit_gate_payload"("p_run" "private"."dataset_review_submit_gate_runs", "p_status_override" "text") TO "service_role";

GRANT ALL ON FUNCTION "private"."cmd_dataset_review_submit_gate_payload"("p_run" "private"."dataset_review_submit_gate_runs", "p_status_override" "text") TO "api_internal_executor";
