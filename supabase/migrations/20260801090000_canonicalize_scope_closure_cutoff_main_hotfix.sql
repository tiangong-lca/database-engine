do $migration$
begin
  -- Production main still owns this helper in public. After the schema cutover
  -- it lives in private and the dev-line migration owns the same canonical
  -- behavior, so a governed main -> dev backmerge must be a safe no-op.
  if to_regprocedure(
