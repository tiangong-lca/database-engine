...
| public SECURITY DEFINER owner/runtime audit | on the exact inventory schema stack, run `python scripts/security_definer_audit.py --check`, `python -m unittest scripts/test_security_definer_audit.py`, and `supabase test db supabase/tests/20260801_security_definer_owner_runtime_audit.sql --local` | validate the JSON Schema and inspect all critical/high inferred-risk signatures plus every role-matrix disposition before #358 | The artifact must retain all 241 signatures: 129 #333 owner/runtime residue, 14 #339 validated RLS-bound facades, and 98 inventory static closures. Static signals are never runtime authorization proof. Issue #352 remains an external blocked platform-owner gate, no migration occurs here, and `contractReady=false` is mandatory. |
