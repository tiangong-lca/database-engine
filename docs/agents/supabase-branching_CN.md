...
lastReviewedNote: "已为 Issue #582 复核：PR workflow 只在精确确认无 supabase/ diff 后跳过 hosted Preview；任何 Supabase 变更仍要求官方 check 与 disposable branch 证据。"
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
- pull-request-only Preview 运行态 job 必须与部署隔离。fork PR 在获得授权前跳过。同仓 PR 先 checkout 事件中的准确 head、验证事件 base/head commit，再只比较 `supabase/`。如果该准确 diff 为空，job 无需 secret、branch 解析或 hosted mutation 即成功，官方 Supabase App 可以返回 `skipped`。只要任一 `supabase/` 路径发生变化，缺少 access token、main-parent ref 或 persistent-Dev ref 就 fail closed。随后仍必须要求准确 PR head 上恰有一个来自官方 Supabase App（id `330661`、slug/owner `supabase`）的成功 check，从准确 dashboard URL 捕获期望 ref，通过固定 CLI `branches list --output json` 独立解析唯一 non-default/non-persistent BranchResponse，并在 ref 相等且不同于 main/Dev 后才执行精确 PostgREST PATCH/readback 与匿名探测。不得 link、push migration、部署 Function/config，也不得指向持久化 Dev 或生产。
...
2. PR 目标分支是 Git `dev`。
3. 当 PR 修改 `supabase/` 时，Supabase GitHub integration 根据已提交目录创建或更新 preview branch。
4. 该 preview 只用于 PR 级验证，绝不是持久化 `dev`。如果事件 base-to-head 的
   `supabase/` diff 精确为空，PR 不需要 preview branch，仓库 job 在路径分类后成功结束。
...
- 精确确认无变更是唯一不要求 Preview check 的路径。只要存在 `supabase/` diff，可接受的 check 必须来自官方 Supabase App id `330661`、slug/owner `supabase`；job 从准确 dashboard `details_url` 捕获期望 ref。固定 CLI 再使用 `branches list --output json`，按 Git branch、PR number、parent project、`is_default=false` 和 `persistent=false` 严格选择唯一 20 字符 `.project_ref`，要求两个 ref 相等且不同于 main/持久化 Dev。Preview 必需时，失败、取消、跳过、stale、neutral、超时、歧义或非官方 check 都 fail closed。
