...
lastReviewedNote: "Reviewed post-merge Issue #310 staging evidence: use the checked-in Edge route parameter profile, retain redacted real staging vectors, and cross-check PGroonga Advisor notices with direct EXPLAIN plans."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| `extracted_text` retirement or Hybrid/Search v2 lexical contract | `supabase db reset`; run `supabase/tests/20260613_hybrid_search_query_terms_escape.sql`, `supabase/tests/20260727_foundation_dataset_semantic_hybrid_search.sql`, and `supabase/tests/20260729_extracted_md_lexical_search_contract_v2.sql` plus the nearby semantic/HNSW regressions | on the exact Preview branch, smoke all seven v2 Hybrid RPCs and `search_processes_latest_v2`, including strict owner-only draft scope; after merge to persistent `dev`, use the read-only benchmark's redacted real vectors and Edge route parameter profile, then compare advisors before Contract cleanup | Expand must build all seven `extracted_md` PGroonga indexes before switching lexical execution, retain old signatures during consumer cutover, and expose one `lexical_weight`. A PGroonga index may still appear as `unused_index` with `idx_scan = 0` after use; require a direct `EXPLAIN (ANALYZE, BUFFERS)` showing the named index before classifying or dropping it. Contract is a separate post-burn-in migration that drops legacy objects with `RESTRICT`; never combine production column/function deletion with the Expand PR. |
...
| scope-closure artifact publication, certificate expiry, download, or GC lifecycle | `scripts/test_scope_closure_base_to_head_upgrade.sh`; `supabase db reset`; run `supabase/tests/20260729_scope_closure_publication_staging_and_gc_control.sql`, `supabase/tests/20260729_scope_closure_owner_artifact_projection.sql`, `supabase/tests/20260729_scope_closure_artifact_retention.sql`, `supabase/tests/20260722_scope_closure_release_binding_e2e.sql`, and `supabase/tests/20260722_data_product_scope_closure_and_task_feed.sql` | run `supabase migration list` and `supabase db lint --level warning`; on the exact Preview branch, confirm the additive migration appears in remote history, prove both public selectors and the temporary one-argument XLSX overload through actor RPC transport, then exercise DB-first staging and Worker GC RPC transport | Prove a populated canonical-base fixture upgrades atomically; fresh publication pre-registers all locators and resolves bundle clientKey to manifest UUID in one finalize; reused publication uploads only the current XLSX and preserves source manifest/bundle lineage; crash recovery sees every locator and no partial ready set. Prove the owner check returns exactly two fixed-order locator-free summaries and exact selector DTO/opacity semantics. Also prove certificate deadlines, non-mutating preview, renewable `SKIP LOCKED` leases with old-token rejection, fresh detail-cleanup reclaim without object re-delete, repeated completion, and compact audit residue. SQL coordinates object I/O but never performs it. |
