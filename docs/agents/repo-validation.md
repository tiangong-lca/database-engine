...
lastReviewedNote: "Reviewed for PR #404 exact semantic qualification: the Root/Reference Review projection retains its least-privilege executor and notification event identity retains its predecessor security envelope, while path/blob/AST classifiers preserve every generic pre-DDL hard deny."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
control-plane changes additionally run `--suite worker-control-plane`. That
suite mandatorily rehearses the populated one-million-row physical move,
failure/retry/lock/WAL budgets, malicious rollback preflight, exact rollback
and roll-forward, then runs the focused pgTAP files. Real PostgREST probes run
at baseline, Expand, restored baseline, and roll-forward: all four public
compatibility relations and both service RPCs must serve `service_role`, deny
anonymous access, and reject the `private` profile. The probe sends explicit
...
mixes benchmark, fixture, Preview, upgrade, and transition-migration assets with
the top-level pgTAP files.
...
| LCA snapshot-family physical Expand | on a disposable loopback stack, run `python scripts/test_lca_snapshot_family_upgrade.py --db-url <local-db-url>`; then run `supabase/tests/20260802_issue_376_lca_snapshot_family_expand.sql` | inspect the runner's JSON summary and repeat the service/anonymous/private-profile REST matrix on the exact Preview head | The runner is destructive only to the selected loopback database. Commit `2e7b3ba` must be an ancestor whose committed migration head is exactly `20260802090000`; a committed checkout may add only head `20260802091342` under `supabase/migrations`, while a base-checkout development run requires that migration in the Git index. Its 10,000+10,000-row fixture proves full-row/PK hashes and OIDs, second-ALTER lock rollback, clean roll-forward, retry and private-drift failure, WAL/time budgets, and committed rollback/roll-forward. Public compatibility remains Expand-only until exact consumer zero and burn-in. |
| LCA result GC additive contract | in a uniquely named disposable loopback stack with explicit ports, run `supabase db reset`; the exact Issue #398 pgTAP file; `python scripts/test_issue_398_result_gc_runtime.py --db-url <explicit-loopback-url> --confirm-isolated-destructive-test`; and the API/private-boundary Node contract | run the exact migration semantic classifier and its mutation-negative suite, then the canonical-local manifest gate | Claims remain disabled. Prove real LOGIN runtime authorization, non-runtime denial, renew/fail, expired claimed/deleting takeover, stale-token rejection, deleted/missing finalize replay, partition/double-fence ordering, update/reference races, PostgREST PGRST202, and zero database/role residue. Current Worker snapshot/job locators are ineligible; Worker #202 must preallocate the result UUID and emit exact `/results/<uuid>/` before attestation. Never hard-code runtime S3 endpoint, scheme, bucket, or prefix in the database. |
| non-core relation family before physical DDL | run `python -m unittest scripts.test_issue_390_pre_ddl_gate scripts.test_issue_390_external_git_tree` and `python scripts/issue_390_external_git_tree.py --check`; for a reviewed cross-repo refresh also run `--verify-external <absolute-workspace-root>`; for the result facade also run `supabase test db --local supabase/tests/20260802_issue_390_result_api_facade_v1.sql` and `python scripts/test_issue_390_result_api_facade_runtime.py`; inspect the checked pre-DDL contract and exact active-source SHAs | collect attributed persistent-Dev runtime evidence for the ADR-defined family window, exercise low-frequency paths, and obtain every named owner sign-off before changing `ddlAuthorized` | A historical browser-role grant is a Contract-cut gate, not permission to expose `private`. The external scanner reads only exact Git objects from canonical origins, scans every regular blob including active generated mirrors/snapshots, retains hashes rather than source lines, and treats unsupported entries, unresolved active-runtime matches, dynamic selectors, stale mirror receipts, or missing source receipts as blockers. Recognized direct-token occurrences are never an exhaustive consumer count. While `ddlAuthorized=false`, migration history is append-only on every governed parent edge and new target-neutral, non-executing static migrations may proceed. Static exclusions include, but are not limited to, top-level DML/CTAS/SELECT, index or exclusion-index builds, validated constraints/partitions, column type or storage rewrites, non-metadata-only column additions, `SET NOT NULL`, custom types/access methods, trigger/rule state changes, and identity/owner switches because existing triggers, views, FDWs, operators, casts, constraints, or access methods may execute unproven code. The checked recursive application-object dependency closure is protected with the seven direct targets. A separately tested additive service-only `api` facade requires an exact reviewed path/blob/classification plus fixed `pglast` PostgreSQL-AST semantic proof in the same first commit as the migration; its signature uses explicit `pg_catalog` types. Moves into exposed `api`/`public` are denied; newly created exposed views must be security-invoker, and exposed routines may neither be security-definer nor reference internal objects. `api.lca_*` and `api.cmd_lca_*` remain protected after creation; opaque/dynamic execution, historical authenticated-access removal, protected service-access removal, and non-service grants on internal/protected or schema-wide exposed targets remain hard denied. HEAD, index, and worktree migration sets use their own contract revision so a later layer cannot authorize an earlier one, and a later commit cannot retroactively authorize an earlier migration. The runtime probe freezes all eight service DTOs, exact anon/authenticated/private denials, HTTP and independent-backend admission races, and zero fixture residue; canonical-local runs it whenever the facade pgTAP contract is selected. Any relevant schema, consumer, identity/ACL/RLS, deployment, PostgREST, or telemetry change resets the observation window. |
| Issue #390 physical qualification harness before DDL authorization | run `python scripts/issue_390_physical_qualification.py --check`, inspect `--print-run-plan`, and run `python -m unittest scripts.test_issue_390_physical_qualification`; an optional `--capture-baseline --db-url <loopback-url> --output <receipt.json>` is read-only and requires an allowlisted owner-capable role with proven full-row visibility | after a reviewed successor binds exact migration/rollback/fixture blobs and every pre-DDL flag is authorized, require independent fresh and populated database receipts with distinct cluster system identifiers plus lock/fault/retry/rollback/roll-forward and measured WAL/time receipts | The v1 harness is intentionally non-authorizing: it binds exact `dev` source and predecessor migration set, four exact ordinary relations, three exact functions, catalog/data invariants, and catalog, policy/publication, rewrite, and case-insensitive lexical dependency closure. The database receipt independently proves database/server/system identity and its exact applied migration set; the Git repository plan remains separate. Sorted canonical SHA-256 PK/full-row digests are bounded to 100,000 rows per target and 120 seconds, otherwise capture fails. Loopback/read-only is not disposability proof. The harness contains no candidate DDL and refuses destructive qualification; no receipt can satisfy static/runtime/owner evidence or authorize Hosted/production mutation. |
| Worker control-plane physical boundary | `python scripts/run_database_contract.py --suite worker-control-plane` on a disposable local stack | run the exact two-clean-stack privileged-lineage qualification; then repeat the read-only REST phase matrix on the exact hosted head | The mandatory suite starts from migration `20260801042600`, loads one million Worker jobs, proves failure atomicity, bounded locks/WAL, catalog/data/OID parity, retry, exact rollback and roll-forward, malicious preflight fail-before-mutation, and real PostgREST behavior in every phase. Skipping reset or Data API is forbidden. |
| identity/collaboration boundary Expand | `supabase db reset`; `supabase test db --local supabase/tests/20260801_identity_collaboration_expand.sql`; `python -m unittest scripts/test_identity_collaboration_expand_static.py scripts/test_database_contract_identity_qualification.py scripts/test_database_contract_targeting.py`; `python scripts/test_identity_collaboration_policy_variants.py`; `python scripts/test_identity_collaboration_data_api.py`; `python scripts/test_identity_collaboration_concurrency.py`; rerun Issue #339 and #333 gates | on one disposable loopback `SUPABASE_WORKDIR`, explicitly opt into `python scripts/run_database_contract.py --suite canonical-local --run-destructive-identity-qualification` and supply every exact pending SECURITY DEFINER transition input required by the runner. The runner resets that selected local stack to exact Issue #355 head `20260801061000`, runs both exact users-policy predecessors, actor-level source/projection RLS parity, retry, rollback/reapply, unknown atomic rejection, reviewed-predecessor routine drift, extra-overload rejection, owner/custom-ACL convergence, exact migration-head/target rollback admission, tamper/partial-state rejection, repeatable zero-state rollback/roll-forward, and five-second lock-failure atomicity, then restores the stack to the current repository head. Qualification and restoration failures both propagate. Then run the global one-million-row populated upgrade. | The canonical runner resolves one stack identity once and explicitly passes its database URL, REST credentials, and workdir to every SQL, Data API, catalog, schema-phase, inventory, lint, and SECURITY DEFINER gate; DB/REST cross-stack composition must fail. Policy fingerprints resolve sorted role names plus `PUBLIC`, never environment-specific OIDs. The policy-variant rehearsal admits only the exact blank/repository and persistent Dev/Production hashes, preserves either source through Expand and rollback/reapply, and rejects a third variant before mutation. The live legacy hash is compatibility-only; tightening remains gated by Next #753 and database-engine #358. All sixteen physical objects remain `public` in Expand. Contract waits for exact-SHA consumer zero and hosted-dev burn-in. |
| Root/Reference Review v2 schema, commands, queue projections, notification identity, or legacy migration | `supabase db reset`; `supabase test db --local supabase/tests/20260731_root_reference_review_v2.sql`; `supabase test db --local supabase/tests/20260802_root_grouped_review_queue.sql`; `python -m unittest scripts.test_issue_323_review_progress_semantic_gate scripts.test_issue_323_review_notification_semantic_gate` | rehearse `root_reference_review_v2_local.sh backup`, restore verification, second-local-copy verification, dry-run, one-row apply, and reconciliation against a disposable production-shaped copy before any hosted cutover | Process alone keeps the numeric Gate. Prove the seven-table state and immutability transitions, exact checksum reuse, append-only scope history, owner-only notifications, advisory Reviewer outcomes, Admin final decisions, preservation of legacy `review_id`/`reviews` projections, reference-name snapshot/fallback behavior, and notification uniqueness by review event. The child projection's SECURITY DEFINER owner must remain the non-login, non-inheriting, non-superuser, non-BYPASSRLS `review_progress_executor`; that role may read only `reviews` and `comments`, receives explicit execute grants only for the exact review/name helpers, and must have no schema-create privilege or table write privileges. The unavoidable PostgreSQL creator-administrator record for `postgres` must grant neither inheritance nor `SET ROLE`, and no API runtime role may be a member. Both reviewed migrations must match their exact first-appearance path/blob/classification, normalized AST, statement sequence, function envelope, relation/ACL set, and mutation-negative semantic tests; the notification replacement must retain its predecessor authorization, audit, owner, ACL, and legacy-only event predicate. These qualifications must not alter any generic pre-DDL hard-deny signal. Distinct reviews for the same dataset version may each notify the owner, while replay of the same review event remains idempotent. Never use the backup runner against production until its local restore markers represent real checks. |
...
| exposed-schema/default-privilege/API-role ACL convergence | `supabase db reset`; run `supabase/tests/20260801_security_acl_expand.sql` and `supabase/tests/20260801_postgres_global_function_defaults.sql`; run `python scripts/test_security_acl_upgrade.py`, `python scripts/test_security_acl_global_function_defaults_upgrade.py`, and `python -m unittest scripts/test_hosted_security_acl.py scripts/test_apply_postgrest_config.py scripts/test_export_database_contract.py` | on the exact hosted Preview or persistent-dev migration head, reconcile configuration only through `scripts/apply_postgrest_config.py`; then run the read-only `scripts/hosted_security_acl.py` with SQL, Management API, and REST credentials; retain its secret-free evidence; rerun Security Advisor | Keep `public` compatibility during Expand. Require the exact `api,public,graphql_public` hosted exposed set in reviewed order; negative `private/util/archive` profiles; opaque publishable keys only in `apikey`; no direct API-role internal schema/routine grants; production-compatible ACLs for the ten drifted LCA relations; zero API-role `MAINTAIN`; and effective defaults computed from PostgreSQL built-ins plus global and per-schema `pg_default_acl`, grouped by ACL identity with grantability folded by `bool_or`. Functions require a global `postgres` revoke; tables/sequences remain application-schema scoped. The PG17 upgrade proof creates a new function in all five schemas plus a non-application scratch schema, proves current-object ACL and row-count parity, retry, failure atomicity, exact global/five-schema layered restore including a custom per-schema grant option, and roll-forward. The rollback snapshot is operator-only: it dynamically removes and validates every non-owner ACL grantee, including unknown custom roles, while retaining the `postgres` owner. A million-row fixture is not applicable because this migration scans or rewrites no application relation; the canonical populated row-count oracle supplies the relevant data-parity proof. Private search facades use a non-login, non-BYPASSRLS executor. The two lifecycle bundle RPCs remain compatibility blockers. `supabase_admin` effective residue stays fail-closed under #352 until an authorized platform-owner path closes it. |
...
| public object inventory/dependency closure | run offline `python scripts/public_inventory_exact_head.py --check` and `python -m unittest scripts.test_public_inventory_exact_head scripts.test_public_inventory_closure`; on an explicit disposable loopback exact-head run `python scripts/public_inventory_exact_head.py --check-live --db-url <url>` and the existing pgTAP dependency closure | regenerate from a clean exact `c5356d2`/`20260803090000` stack with `--refresh --db-url <url>` and prove byte/hash equality; compare blank and populated exact-head catalogs with `--compare-catalogs --db-url <blank> --other-db-url <populated>` | Issue #405 requires 397 identities (49 tables + 12 views + 336 functions), exactly-once partition `9+37+117+230+4`, and a 388-entry blocked identity-only DROP checklist. The reviewed SHA-256 migration tree and live exact applied-version set are mandatory. Missing/unknown/duplicate/count/hash/schema/target/status/counterpart or overload drift fails closed. `physicalMoved`, `compatPresent`, `adapterOnly`, and `retired` are independently derived facts. The tool never emits DDL and `contractReady=false` is invariant; the immutable #338 hash remains genesis lineage only. |
