...
lastReviewedNote: "Reviewed for Issues #412 and #323: the business-neutral static-SQL SECURITY DEFINER gate and latest-event notification behavior have separate proof boundaries."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Root/Reference Review v2 schema, commands, queue projections, notification identity, or legacy migration | `supabase db reset`; `supabase test db --local supabase/tests/20260731_root_reference_review_v2.sql`; `python -m unittest scripts.test_issue_390_pre_ddl_gate` | rehearse `root_reference_review_v2_local.sh backup`, restore verification, second-local-copy verification, dry-run, one-row apply, and reconciliation against a disposable production-shaped copy before any hosted cutover | Process alone keeps the numeric Gate. Prove the seven-table state and immutability transitions, exact checksum reuse, append-only scope history, owner-only notifications, advisory Reviewer outcomes, Admin final decisions, and preservation of legacy `review_id`/`reviews` projections. Review events keep one latest-event notification under the existing six-column identity: rejection updates the prior notification row and repeated delivery remains idempotent. The replacement remains a private, statically parseable `LANGUAGE SQL` SECURITY DEFINER function with `pg_catalog, pg_temp` search path and fully qualified relation access, passes the Issue #412 business-neutral structural pre-DDL rule, and removes the superseded per-event unique index so the six-column latest-event identity is the single conflict boundary. Never use the backup runner against production until its local restore markers represent real checks. |
