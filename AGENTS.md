...
lastReviewedNote: "Reviewed for Issue #566: OAuth client authorization is database-owned through a revocation registry, restrictive relation-read RLS, and the manifest-backed PostgREST gate; actor writes stay on three narrow command RPCs with a separate capability."
related:
  - .docpact/config.yaml
...
- Supabase Auth owns OAuth client credentials and refresh tokens; the database stores only public `client_id`, enabled/revoked state, capability grants, and audit history. First-party sessions without `client_id` retain existing `auth.uid()` behavior. OAuth relation reads require `DB-CORE-READ-01`; raw table DML remains ACL-closed, while actor create/save/delete uses the existing command RPCs under `DB-CORE-WRITE-01`; every relation/RPC route fails closed through restrictive RLS plus the manifest-backed PostgREST pre-request hook
