...
declare
  ...
  v_review_record private.reviews%rowtype;
  v_review_json jsonb;
  v_review_row jsonb;
  v_team_name jsonb;
  v_user_meta jsonb;
...
begin
  ...
  while exists (select 1 from cmd_review_submit_queue) loop
    ...
    v_current_row := api.cmd_review_get_dataset_row(
    ...
    if v_current.is_root then
      ...
      if v_current_state_code >= 100 then
        if v_current.table_name = 'processes' then
          v_paired_model_exists := api.cmd_review_get_dataset_row(
            'lifecyclemodels',
            v_current.dataset_id,
            v_current.dataset_version,
            false
    ...
    for v_ref in (
      select *
      from api.cmd_review_extract_refs(coalesce(v_current_row->'json_ordered', '{}'::jsonb))
      union
      select *
    ...
    if v_current.table_name = 'lifecyclemodels' then
      if v_current.is_root then
        v_paired_process_exists := api.cmd_review_get_dataset_row(
          'processes',
          v_current.dataset_id,
          v_current.dataset_version,
          false
  ...
  v_review_json := jsonb_build_object(
    'data', jsonb_build_object(
      'id', p_id,
      'version', p_version,
      'name', api.cmd_review_get_dataset_name(p_table, v_root_row)
    ),
  ...
  loop
    v_updated_reviews := api.cmd_review_append_review_ref(v_current.reviews, v_review_id);
...
ALTER FUNCTION "private"."cmd_review_submit_without_gate_issue304_legacy"("p_table" "text", "p_id" "uuid", "p_version" "text", "p_audit" "jsonb") OWNER TO "postgres";

REVOKE ALL ON FUNCTION "private"."cmd_review_submit_without_gate_issue304_legacy"("p_table" "text", "p_id" "uuid", "p_version" "text", "p_audit" "jsonb") FROM PUBLIC;

GRANT ALL ON FUNCTION "private"."cmd_review_submit_without_gate_issue304_legacy"("p_table" "text", "p_id" "uuid", "p_version" "text", "p_audit" "jsonb") TO "api_internal_executor";
