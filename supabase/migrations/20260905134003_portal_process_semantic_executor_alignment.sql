-- Database #620: execute the Process V2 semantic leaf as the fixed public Portal
...
do $portal_process_semantic_executor_alignment_guard$
declare
  v_function_sha text;
begin
  ...
  if v_function_sha is distinct from
     ...
     or not exists (
       ...
       where routine.oid =
         ...
         and routine.proowner = 'api_internal_executor'::regrole
         and routine.prosrc ~ 'portal_projection_semantic_process_v2'
     )
     or not exists (
       select 1
    ...
    raise exception 'Portal Process semantic executor prerequisites drifted'
      using errcode = '55000';
  end if;
end
$portal_process_semantic_executor_alignment_guard$;
...
grant select (embedding_ft)
  on table public.processes
  to portal_public_executor;

alter function private.portal_projection_semantic_process_v2(
  extensions.vector,
  jsonb
...
comment on function private.portal_projection_semantic_process_v2(
  ...
  'Returns at most 200 exact public Process versions under portal_public_executor RLS: exact distance over at most 2000 indexed geography/access candidates, otherwise strict iterative HNSW.';
...
do $verify_portal_process_semantic_executor_alignment$
declare
  v_function_sha text;
begin
  select pg_catalog.encode(
  ...
  if v_function_sha is distinct from
    ...
    raise exception 'Portal Process semantic executor alignment drifted'
      using errcode = '55000';
  end if;
end
$verify_portal_process_semantic_executor_alignment$;
