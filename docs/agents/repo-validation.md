...
| guarded atomic owner-draft FP/UG alias plan RPC | `supabase db reset`; run `supabase/tests/20260711_guarded_dataset_alias_batch.sql` and the legacy `supabase/tests/20260404_dataset_command_rpcs.sql` | prove authenticated callers can execute only `cmd_dataset_alias_plan_guarded`; the outer function alone has `statement_timeout=45s` in addition to the empty search path and five-second lock wait, while the internal batch has no separate statement budget; run a real authenticated Preview Database REST probe whose runtime exceeds the role timeout plus the production-shaped 52-row/59-exchange plan; retain the exact `dataset-alias-plan.v1` envelope with one plan hash/operation ID, `target_visibility=owner_draft`, and ordered time plus length-time batches; prove the 25-row/20-exchange and 27-row/39-exchange scopes; a non-locking actor-owned `state_code=0` support/action preflight before broad locks; public/foreign/mixed-parent rejection; embedded UUID/version identity; allowed-path reconstruction; exact decimal factors; canonical exchange hashes; exact live closures; input bounds; two complete batch audit sets plus one plan proof; forced timeout or second-dimension failure rollback of the first dimension and all audits; and identical whole-plan lost-response replay without duplicate audits. Before Preview benchmarking, re-freeze whole physical-table flow/process and nested-node counts, build one background fixture no smaller than those counts, and retain real extracted-text and derivative trigger behavior. Capture `EXPLAIN (ANALYZE, BUFFERS, WAL, SETTINGS)` proving both candidate predicates use their `json_ordered` `jsonb_path_ops` GIN indexes and exact expansion is bounded to candidate rows. Run at least 20 single-concurrency authenticated REST successes against that fixed production-cardinality background, split across at least three independent exact target/support logical reloads. Before each group, read back the unchanged background cardinality and the complete reloaded target/support state; report the first execution after each reload separately. Require p95 at most 30 seconds and maximum at most 35 seconds. | The 45-second exception is a PostgREST-hoisted whole-request budget, not a role-wide relaxation or a per-batch reset; direct SQL and pgTAP cannot prove the REST hoist. Rebuilding the entire background corpus between groups is not required: it primarily measures fixture I/O and can exhaust disposable Preview WAL without adding guarded-RPC evidence. The plan RPC is independent of support publication and never changes visibility. Only the exact 52-row/59-exchange two-dimension plan is an authenticated API; the dimension executor has no authenticated execute grant. After the preflight, short `SHARE ROW EXCLUSIVE` locks on `flowproperties`, `flows`, and `processes` prevent support-parent and mutation-closure phantoms, and locked checks repeat ownership/state validation. GIN predicates only materialize candidate keys; every semantic check remains an exact `json_ordered` recheck, singleton object and array encodings are both covered, and a drifted legacy `json` value cannot hide a parent. The private immutable index helper remains unavailable to anon/authenticated but executable by service-role so trusted direct writes do not fail index maintenance. Additional same-owner draft parents of target support are allowed because they are not mutated; any public, foreign, or non-draft parent fails closed. Both dimensions execute in one exception subtransaction: any timeout, second-batch, closure, snapshot, row, mutation, amount, audit, or proof failure must leave all 52 datasets and every audit unchanged. |
| guarded owner-draft process derivative rebuild | `supabase db reset`; run `supabase/tests/20260714_guarded_dataset_derivative_rebuild.sql` plus the nearby webhook, embedding queue, and dataset-command regression assertions | prove the three public RPC ACLs and empty search paths; exact one-action snapshot/admission/read envelopes; owner/state/hash drift rejection with zero audit/request/queue effects; exactly-once request/action/summary admission plus terminal audit; active primary update/delete freeze; queued PGMQ and already-claimed array-body `pg_net` quarantine, including mixed-batch retry semantics that preserve unrelated PGMQ messages; a 420-second visibility/drain window; private Markdown staging input; pre-publication freshness validation and one-update Markdown/vector publication; stale completion rollback that preserves the old pair; transaction-bound private commit permits; real deferred enqueue timestamps; terminal-failure fresh-plan retry; request fairness/error isolation; independent owner readback; and no direct authenticated access to coordinator, queues, worker helpers, tables, or permits | Apply means queued admission only. The short admission `SHARE ROW EXCLUSIVE` lock serializes pre-fence process writers and has a five-second timeout. A target-bearing mixed HTTP batch is canceled as a unit, so unrelated jobs remain queued but may wait for their visibility timeout before retry. Nonterminal drain/failure phases retain the primary fence. `completed`, `stale`, or `failed` may release it only after request-specific work and pre-existing worker windows are drained. A failure preserves the prior derivative pair. The database promises exactly-once admission/audit and idempotent effects, not exactly-once external invocation. |
