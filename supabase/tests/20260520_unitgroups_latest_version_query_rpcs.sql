...
select is(
  (
    select version::text
    from api.get_latest_unitgroup_versions(10, 1, 'tg', '16000000-0000-0000-0000-000000000046')
...
select is(
  (
    select count(*)
    from api.get_latest_unitgroup_versions(
      10,
      1,
      'tg',
      '16000000-0000-0000-0000-000000000046',
...
select is(
  strpos(
    lower(pg_get_functiondef('api.get_latest_unitgroup_versions(bigint,bigint,text,text,uuid,integer,text,text)'::regprocedure)),
...
select is(
  (
    select version::text
    from api.pgroonga_search_unitgroups_latest(
...
select is(
  strpos(pg_get_functiondef('api.pgroonga_search_unitgroups_latest(text,jsonb,bigint,bigint,text,text,uuid,integer)'::regprocedure), 'user_id::text = this_user_id'),
  0,
...
select ok(
  exists (
    ...
    where p.oid = 'api.pgroonga_search_unitgroups_latest(text,jsonb,bigint,bigint,text,text,uuid,integer)'::regprocedure
