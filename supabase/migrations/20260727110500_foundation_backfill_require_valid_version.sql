-- An existing staging FlowProperty has a blank primary-key version and no
-- version inside its authored JSON. Such a row cannot form the stable
-- id/version identity required by the extraction and embedding workers. Keep
-- it auditable as a data-quality exception, but do not enqueue a terminally
-- invalid compact job during historical backfill.
...
comment on function public.cmd_dataset_semantic_backfill(text, integer, uuid, text, boolean) is
  'Service-only bounded cursor RPC that enqueues missing foundation-dataset derivatives and skips rows without a stable nonblank id/version worker identity.';
