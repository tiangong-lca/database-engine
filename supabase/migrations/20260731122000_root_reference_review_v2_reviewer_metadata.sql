-- Reviewer metadata on Process/Lifecycle Model roots may introduce references.
-- The v2 branch shares Reference Reviews and appends a new immutable scope
-- snapshot. Legacy rows continue through the retained implementation.

alter function public.cmd_review_submit_comment(
  uuid, jsonb, integer, jsonb
) rename to cmd_review_submit_comment_pre_v2;

revoke all on function public.cmd_review_submit_comment_pre_v2(
  uuid, jsonb, integer, jsonb
) from public, anon, authenticated, service_role;

create function public.cmd_review_submit_comment(
  p_review_id uuid,
  p_json jsonb,
...
begin
  ...
  if v_review.review_kind = 'reference'
    ...
    return public.cmd_simple_review_submit_decision(
      p_review_id,
  ...
  if p_comment_state = 1 then
    for v_ref in
      select *
      from public.cmd_review_extract_refs(coalesce(p_json, '{}'::jsonb))
    loop
      if public.cmd_review_ref_type_to_table(v_ref.ref_type) is not null then
        v_ref_roots := v_ref_roots || jsonb_build_array(jsonb_build_object(
          'table', public.cmd_review_ref_type_to_table(v_ref.ref_type),
    ...
    loop
      ...
      v_checksum := public.review_revision_fingerprint_v1(
        v_target.table_name,
        v_target.dataset_row
      );
      ...
      if not exists (
        select 1
        from public.review_scope_current_items_v1(v_review.scope_history) as current_item
...
revoke all on function public.cmd_review_submit_comment(
  uuid, jsonb, integer, jsonb
) from public, anon;
grant execute on function public.cmd_review_submit_comment(
  uuid, jsonb, integer, jsonb
...
create or replace function private.review_v2_kind_guard()
returns trigger
language plpgsql
security definer
set search_path = ''
...
revoke all on function private.review_v2_kind_guard()
  from public, anon, authenticated, service_role;

drop trigger if exists reviews_v2_kind_guard on public.reviews;
create trigger reviews_v2_kind_guard
before insert or update on public.reviews
for each row execute function private.review_v2_kind_guard();

create or replace function private.review_v2_comment_guard()
returns trigger
language plpgsql
security definer
set search_path = ''
...
revoke all on function private.review_v2_comment_guard()
  from public, anon, authenticated, service_role;

drop trigger if exists comments_v2_kind_guard on public.comments;
create trigger comments_v2_kind_guard
before insert or update on public.comments
for each row execute function private.review_v2_comment_guard();
