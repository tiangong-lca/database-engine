...
lastReviewedNote: "Reviewed for Issues #551/#552: 290-migration reset, forced-RLS CAS indexability, one-code-point literal correctness/p95, duplicate pressure, and unified CLI restart/Preview gates."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Portal catalog-summary and literal-Search executability | blank reset plus `supabase/tests/20260825_portal_public_catalog_v1.sql` and `20260826_portal_candidate_first_search.sql`; run summary and full release benchmarks with 20 samples | after Dev/Main deployment, resolve examples dynamically through the anonymous summary, execute each declared Search RPC, and probe one-code-point Process/Flow literals; capture a sanitized forced-RLS plan and log only kind/status/latency/count | Classification examples require a trimmed public code of at least four characters and prefer Process. CAS selection keeps at most 64 history-unique values and rechecks latest identity. The row-neutral Portal policy is valid only with forced RLS, the exact validated state-100/200 CHECK, and the live assertion; CAS equality must be an exact `Index Cond`. Multi-code-point and escaped literal Search retains PGroonga. Only `%` + one unescaped code point + `%` uses ACL-closed helpers that disable index/bitmap paths locally and run parallel `strpos`, because TokenBigram may produce no candidate and heap recheck cannot recover a false negative. Summary p95 stays <=250 ms under 10,000 duplicate CAS rows; CAS/classification and one-code-point Process/Flow p95 stay <2 seconds and every Search/Hybrid sample remains under its existing hard gate. Do not raise timeouts, log queries, infer substitutes, or add an index without writer/storage evidence. |
...
use the isolated parallel `strpos` correctness helpers. Their function-local
GUCs disable index, index-only, and bitmap paths without changing the caller;
both labels keep p95 below two seconds,
