-- Operator-only, one-legacy-review-at-a-time migration command. The original
-- review remains immutable with review_kind NULL; the returned old/new mapping
-- is written to command_audit_log and to the caller's local migration manifest.

create or replace function public.cmd_review_migrate_legacy_v2(
  p_legacy_review_id uuid,
  p_new_root_review_id uuid,
...
as $$
declare
  v_legacy public.reviews%rowtype;
  v_root_row jsonb;
  v_table text;
...
begin
  ...
  if exists (
    select 1
    from public.reviews as migrated
...
alter function public.cmd_review_migrate_legacy_v2(
  uuid, uuid, jsonb
) owner to postgres;
revoke all on function public.cmd_review_migrate_legacy_v2(
  uuid, uuid, jsonb
) from public, anon, authenticated;
grant execute on function public.cmd_review_migrate_legacy_v2(
  uuid, uuid, jsonb
) to service_role;
