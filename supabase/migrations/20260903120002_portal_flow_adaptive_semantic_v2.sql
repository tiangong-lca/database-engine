-- Database #603: avoid deep filtered HNSW traversal when an already
-- synchronized Flow facet proves that the exact-version candidate universe is
...
do $portal_flow_adaptive_semantic_cutover_guard$
declare
  v_function_sha text;
...
begin
  ...
  if v_function_sha is distinct from
       'f55494e1f5810da29714aa63d335cd3d43b8d474aef1e9d27c7927b7dacede6c'
    ...
    raise exception 'Portal Flow adaptive semantic prerequisites drifted'
      using errcode = '55000';
  end if;
end
$portal_flow_adaptive_semantic_cutover_guard$;

comment on index private.portal_catalog_facet_flow_geography_v1_idx is
  'Exact Flow id/version candidates for bounded Portal semantic geography routing.';
comment on index private.portal_catalog_facet_flow_access_level_v1_idx is
  'Exact Flow id/version candidates for bounded Portal semantic access-level routing.';
...
create or replace function private.portal_projection_semantic_flow_v2(
...
comment on function private.portal_projection_semantic_flow_v2(
  ...
  'Returns at most 200 exact Flow versions: exact distance over at most 2000 indexed geography/access candidates, otherwise strict iterative HNSW.';

reset role;
revoke create on schema private from api_internal_executor;
revoke api_internal_executor from postgres;

do $verify_portal_flow_adaptive_semantic_cutover$
declare
  v_function_sha text;
  v_index_fingerprints text[];
begin
  ...
  if v_function_sha is distinct from
    ...
    raise exception 'Portal Flow adaptive semantic cutover drifted'
      using errcode = '55000';
  end if;
end
$verify_portal_flow_adaptive_semantic_cutover$;
