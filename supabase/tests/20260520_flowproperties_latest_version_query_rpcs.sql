...
select is(
  (
    select version::text
    from api.get_latest_flowproperty_versions(10, 1, 'tg', '16000000-0000-0000-0000-000000000046')
...
select is(
  (
    select count(*)
    from api.get_latest_flowproperty_versions(
      10,
      1,
      'tg',
      '16000000-0000-0000-0000-000000000046',
...
select is(
  strpos(
    lower(pg_get_functiondef('api.get_latest_flowproperty_versions(bigint,bigint,text,text,uuid,integer,text,text)'::regprocedure)),
...
select is(
  (
    select version::text
    from api.pgroonga_search_flowproperties_latest(
...
select is(
  strpos(pg_get_functiondef('api.pgroonga_search_flowproperties_latest(text,jsonb,bigint,bigint,text,text,uuid,integer)'::regprocedure), 'user_id::text = this_user_id'),
  0,
  'flow property latest search does not cast user_id on the my-data predicate'
);

select ok(
  to_regclass('public.flowproperties_extracted_md_pgroonga') is not null,
  'flow property latest PGroonga search has an extracted_md index'
);
...
select ok(
  ...
  'flow property latest PGroonga search matches query_text against extracted_md'
);
...
select ok(
  exists (
    ...
    where p.oid = 'api.pgroonga_search_flowproperties_latest(text,jsonb,bigint,bigint,text,text,uuid,integer)'::regprocedure
