...
class PublicInventoryExactHeadTest(unittest.TestCase):
    ...
    def test_exact_counts_and_partition_are_closed(self) -> None:
        ...
        self.assertEqual(self.inventory["counts"], {"function": 336, "table": 46, "view": 15})
    ...
    def test_state_model_distinguishes_move_compat_adapter_and_retirement(self) -> None:
        ...
        self.assertEqual(sum(state["physicalMoved"] for state in states), 10)
        self.assertEqual(sum(state["compatPresent"] for state in states), 47)
        self.assertEqual(sum(state["adapterOnly"] for state in states), 39)
