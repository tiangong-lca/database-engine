...
lastReviewedNote: "Reviewed the guarded support publication proof after adding independent review-admin approval; validation now covers unforgeable exact-snapshot approval and approval-bound replay."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| guarded dataset maintenance publish RPC | `supabase db reset`; run `supabase/tests/20260711_guarded_dataset_publish.sql` and the legacy `supabase/tests/20260404_dataset_command_rpcs.sql` | include owner-scoped lock/non-disclosure, draft-state rejection, stale `modified_at` and `json_ordered` preconditions, authenticated-only execute grants, independent system-team `review-admin` approval, owner self-approval rejection, raw audit-write denial, approval-role revalidation, indexed approval/publish correlation bound to the exact timestamp and payload, same-action replay, independently published row rejection, and rollback after a forced publish-audit insert failure | The guarded RPCs are limited to `unitgroups` and `flowproperties`. Approval and publication are separate authenticated actions. Publication must bind the exact immutable approval audit id; the owner update and publication audit insert must remain one transaction so a retry after a lost response can be proven by the committed audit row. JSON responses expose bigint audit ids as strings. |
