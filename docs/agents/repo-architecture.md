...
New certificate-grade Scope Closure normalization canonicalizes omitted and supported legacy `closed/open/cutoff` technosphere boundary inputs to `cutoff` before requested-scope, policy, snapshot, and request fingerprints are computed. Unknown values remain invalid. Historical frozen requests and artifacts are not rewritten; the matching Worker contract accepts only the new canonical cutoff input.
