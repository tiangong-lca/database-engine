...
do $verify_portal_facet_backfill_80_bf_process_parity$
begin
  if exists (
    select 1
    from private.portal_catalog_search_rows_v1 as projection
    ...
    raise exception 'Portal facet projection backfill 80_bf is incomplete'
      using errcode = '55000';
  end if;
end
$verify_portal_facet_backfill_80_bf_process_parity$;

do $verify_portal_facet_backfill_80_bf_flow_parity$
begin
  if exists (
    select 1
    from private.portal_catalog_search_rows_v1 as projection
...
$verify_portal_facet_backfill_80_bf_flow_parity$;

do $verify_portal_facet_backfill_80_bf_process_extra$
begin
  if exists (
    select 1
...
$verify_portal_facet_backfill_80_bf_process_extra$;

do $verify_portal_facet_backfill_80_bf_flow_extra$
begin
  if exists (
    select 1
    from private.portal_catalog_facet_rows_v1 as facet
...
$verify_portal_facet_backfill_80_bf_flow_extra$;
