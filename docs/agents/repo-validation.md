...
lastReviewedNote: "Reviewed for Issues #407/#408: exact-head proof advances to migration 20260803163000 and canonical offline validation adds exact 7/9 capability generations, candidate Worker evidence, and external caller exclusion."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| public object inventory/dependency closure | run offline `python scripts/public_inventory_exact_head.py --check` and `python -m unittest scripts.test_public_inventory_exact_head scripts.test_public_inventory_closure`; on an explicit disposable loopback exact-head run `python scripts/public_inventory_exact_head.py --check-live --db-url <url>` and the existing pgTAP dependency closure | regenerate from a clean exact `e1d467c`/`20260803163000` stack with `--refresh --db-url <url>` and prove byte/hash equality; compare blank and populated exact-head catalogs with `--compare-catalogs --db-url <blank> --other-db-url <populated>` | Issue #405 requires 397 identities (49 tables + 12 views + 336 functions), exactly-once partition `9+37+117+230+4`, and a 388-entry blocked identity-only DROP checklist. The reviewed SHA-256 migration tree and live exact applied-version set are mandatory. Missing/unknown/duplicate/count/hash/schema/target/status/counterpart or overload drift fails closed. `physicalMoved`, `compatPresent`, `adapterOnly`, and `retired` are independently derived facts. The tool never emits DDL and `contractReady=false` is invariant; the immutable #338 hash remains genesis lineage only. |
| Issue #408 Worker runtime permission B0 | run `ISSUE408_WORKER_REPO=<clean-exact-worker-checkout> python -m unittest scripts.test_issue_408_worker_runtime_permission scripts.test_issue_408_worker_source_consumer scripts.test_issue_408_external_caller_registry`; run all three checkers and set `ISSUE408_WORKSPACE_ROOT=<exact-workspace-root>` for the cross-repo source readback | after PR #208 and #409 merge, regenerate against their canonical merge SHAs; require hosted PG17 catalog/login receipts, runtime SQL trace, dynamic/unqualified selector closure, and exact deployment membership evidence before any capability expansion | B0 is non-authorizing. It freezes capability-role/object grants as exact 7/9 generations and rejects arbitrary supersets. Deployment LOGIN memberships are deliberately outside that hash and require a separate run-owned exact receipt. The checked Git catalog proves exact object ACL/defaults, but cannot encode role attributes or creator membership; those remain bound to the exact migration self-guard plus hosted PG17 receipt. The candidate Worker artifact identifies its open-PR ref, classifies every qualified token (including explicit non-SQL false positives), leaves unqualified/parser and telemetry residue blocking, and keeps both `sourceArtifactComplete=false` and `contractReady=false`. Database CI checks the exact public Worker tree plus external registry structure/hash; private-repo Git-object replay is a separately required controlled-workspace proof and is never inferred from artifact-only CI. |
