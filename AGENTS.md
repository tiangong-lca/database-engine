...
lastReviewedNote: "Reviewed for Issue #582: direct OAuth clients remain exact and DCR-disabled; Preview runtime proof is required only for deployable Supabase inputs, while repository-only changes accept the official skip."
related:
  - .docpact/config.yaml
