...
-- Production main still owns these coordinator objects in public.  The same
-- migration is later back-merged after the dev schema cutover, where it must
-- be a parse-safe no-op before the private-schema reconciliation migration.
DO $hotfix$
BEGIN
  IF to_regclass('public.worker_jobs') IS NOT NULL
     AND to_regclass('public.lcia_scope_closure_checks') IS NOT NULL THEN
    EXECUTE $ddl$
CREATE OR REPLACE FUNCTION public.lcia_scope_closure_bundle_binding_matches(p_check public.lcia_scope_closure_checks, p_bundle public.worker_job_artifacts)
 RETURNS boolean
 LANGUAGE sql
...
begin
  ...
  if v_snapshot.id is null
     ...
     or not public.lcia_scope_closure_bundle_binding_matches(v_check, v_bundle)
...
begin
  ...
  if v_report.id is null
     ...
     or not public.lcia_scope_closure_bundle_binding_matches(new, v_bundle)
...
BEGIN
  IF to_regclass('public.worker_jobs') IS NOT NULL THEN
    EXECUTE $ddl$
CREATE OR REPLACE FUNCTION public.lcia_scope_closure_evidence_usable(p_check public.lcia_scope_closure_checks)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
  ...
  select
    ...
    and exists (
      ...
      where report.id = (p_check).report_artifact_id
        ...
        and public.lcia_scope_closure_bundle_binding_matches(p_check, bundle)
...
BEGIN
  IF to_regclass('public.worker_jobs') IS NOT NULL THEN
    EXECUTE $ddl$
CREATE OR REPLACE FUNCTION public.svc_lcia_scope_closure_build_binding_without_expiry(build_worker_job_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
$ddl$;
    ...
    EXECUTE $ddl$
      REVOKE ALL ON FUNCTION public.lcia_scope_closure_bundle_binding_matches(
        public.lcia_scope_closure_checks,
        public.worker_job_artifacts
      ) FROM PUBLIC
    $ddl$;

    -- Historical terminal facts keep their timestamps and event history.  The
    -- canonical job row alone is normalized to the completed-progress invariant.
    UPDATE public.worker_jobs
    SET progress = 1
