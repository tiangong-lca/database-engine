...
lastReviewedNote: 'Reviewed for Database #603: version-aware Portal Flow semantic retrieval routes small geography/access-level candidate sets to exact distance evaluation and retains the existing bounded HNSW path for broad searches.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
semantic path, also run `20260903_portal_flow_adaptive_semantic_v2.sql` and the
three adjacent semantic/HNSW governance suites from the proof matrix. Prove
...
`scan_mem_multiplier=2` settings. Real public-data relevance and deployed
