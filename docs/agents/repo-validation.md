...
lastReviewedNote: "Reviewed for Issue #543: 277-migration reset, summary-selected classification/Flow CAS execution, summary-plan isolation, unchanged Search budgets, and anonymous Preview transport gates."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Portal catalog-summary executable examples | blank reset plus `supabase/tests/20260825_portal_public_catalog_v1.sql`; run the summary cardinality benchmark with 20 samples | after Dev/Main deployment, resolve examples dynamically through the anonymous summary and execute each through its declared public Search RPC; log only query kind/dataset kind/status/latency/count | Classification examples require a trimmed public code of at least four characters, prefer Process evidence, and are omitted when no eligible evidence exists. Valid Flow CAS uses only the length-isolated exact partial-card B-tree branch with latest-version recheck; the unconstrained summary selector must retain `portal_catalog_summary_eligibility_v1_idx`, while ordinary lexical/UUID/invalid-CAS text retains PGroonga. Summary p95 stays <=250 ms and every emitted classification/CAS example returns at least one item, p95 <2 seconds, every sample <8 seconds. The CAS index requires representative build bytes/time and incremental writer evidence. Do not raise either timeout, log the query, or infer a substitute. |
...
it must not enter the PGroonga pattern path. The benchmark's temporary writer
