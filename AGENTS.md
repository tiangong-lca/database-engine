...
- a non-core relation family with a historical browser-role grant must first use a checked `*_pre_ddl.v1.json` contract: no relation-moving migration may exist while `ddlAuthorized=false`, and compatibility must not be preserved by granting browser roles access to `private`; authorization requires exact static, attributed runtime, and owner evidence
