...
lastReviewedNote: "Updated for Issue #422: when native Git dev synchronization remains bound, persistent-Dev proof includes the Edge-owned exact-SHA recovery and readback gate."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| `.github/workflows/supabase-dev.yml` | inspect and parse YAML; run `python scripts/test_resolve_migration_head.py` and `python scripts/test_supabase_dev_workflow_contract.py`; regenerate `supabase/workspace/database.types.ts` from the full local migration state; assert the hosted job links the configured Dev project, runs exactly one `supabase db push --include-all`, contains no pinned migration head, Functions deploy/delete, `config push`, or Management API mutation | prove local contract success gates the remote deploy, generated type drift is rejected, the expected head is derived from the checkout, and post-deploy verification performs only Management API GET plus read-only Data API probes: profile-less `public`, explicit `api`, rejected `private`, and retired `public` RPC; if the native Git `dev` binding remains, separately prove the exact reviewed Edge SHA's complete managed inventory was deployed through the Edge repo and passed source-path/status/auth readback plus representative behavior probes | GitHub Actions is the sole persistent-Dev migration deployer. Prefer detaching the native Git `dev` binding; while it remains, the database workflow must stay database-only and persistent-Dev completion additionally requires the Edge-owned recovery gate in `docs/agents/supabase-branching.md`. |
...
- persistent remote `dev` proof happens after merge into Git `dev`; while the native Git binding remains, it ends only after the exact-Edge-SHA recovery and readback gate passes
