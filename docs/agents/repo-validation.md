...
lastReviewedNote: "Reviewed through Issues #339/#341, #346, and #351: retain ACL convergence, opaque-key Worker probe qualification, million-row populated-upgrade qualification, and persistent-dev rollback/readback proof."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| global schema boundary or populated-upgrade qualification | on an isolated local stack and clean exact commit, run `python scripts/test_production_equivalent_upgrade.py --evidence-out <absolute-path-outside-worktree>` with the contract's one-million-row default | inspect the emitted redacted manifest; compare fixture/upgrade seconds and migration WAL to the checked budgets; retain exact commit, migration SHA-256, base/head history, row/PK/content oracle hashes, per-migration fault results, lock-timeout recovery, compatible-reader proof, role matrix, and catalog-invariant hash in the PR | The harness is destructive only to the selected local stack and must never use `--linked` or a hosted URL. Its fixture spans identity, review, notification, audit, active/leased/retry/failed Worker jobs, artifacts, package/cache, release, and closure. Every pending migration must roll back cleanly under injected failure; an incompatible lock must fail within five seconds; the real roll-forward and no-op retry must preserve all physical-table rows, constraints, ACL/RLS, policies, triggers, and publication membership. Update the base/head contract deliberately when migration scope advances. |
