...
lastReviewedNote: "Reviewed Issue #308 rollout compatibility: fixed locator-free owner summaries, strict selector DTOs, one-argument XLSX equivalence, lifecycle opacity, fresh-process cleanup reclaim, additive Preview migration, and advisor proof are explicit."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| scope-closure artifact, certificate expiry, download, or GC lifecycle | `supabase db reset`; run `supabase/tests/20260729_scope_closure_owner_artifact_projection.sql`, `supabase/tests/20260729_scope_closure_artifact_retention.sql`, `supabase/tests/20260722_scope_closure_release_binding_e2e.sql`, and `supabase/tests/20260722_data_product_scope_closure_and_task_feed.sql` | run `supabase migration list` and `supabase db lint --level warning`; on the exact Preview branch, confirm the follow-up migration appears in remote history, prove both public selectors and the temporary one-argument XLSX overload through actor RPC transport, then prove Worker object-delete and fresh-process detail-cleanup claims | Prove the owner check returns exactly two fixed-order locator-free artifact summaries across pending/ready/expired/deleted/failed/missing states, with exact public keys and no locator/service detail; prove exact 11-field `closure_report_xlsx` and `closure_issue_manifest` download descriptors, semantic filenames, format/media pairs, linked locator integrity, invalid-selector rejection, owner-visible `410 closure_report_expired`, deleted/unready owner 404, cross-owner opaque 404, and exact legacy-overload equivalence with the XLSX selector. Also prove certificate `valid_until` bounds, service-only `SKIP LOCKED` claims, `gcPhase` / `objectDeleteRequired`, new-token reclaim after partial tombstone cleanup, repeated final completion, and compact counts/content hashes. SQL coordinates GC but never deletes Storage objects. |
