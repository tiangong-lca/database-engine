...
lastReviewedNote: "Reviewed Issue #308 scope-closure evidence retention: the exact reset, lifecycle/ACL/build/GC SQL suites, Preview upload/delete proof, and known advisor baseline are explicit."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| scope-closure artifact, certificate expiry, download, or GC lifecycle | `supabase db reset`; run `supabase/tests/20260729_scope_closure_artifact_retention.sql`, `supabase/tests/20260722_scope_closure_release_binding_e2e.sql`, and `supabase/tests/20260722_data_product_scope_closure_and_task_feed.sql` | run `supabase migration list` and `supabase db lint --level warning`; on the exact Preview branch, prove the Worker can upload report, complete-machine-result, and closure-bundle artifacts with the database-assigned seven-day deadline, then delete a disposable object before completing its GC claim | Prove actor-bound ready download metadata, owner-visible `410 closure_report_expired`, cross-owner opaque 404, certificate `valid_until` bounded by all three evidence artifacts, build enqueue/claim expiry fences, service-only `SKIP LOCKED` claims, failure retry, missing-object idempotency, bounded detail deletion, and retained counts/content hashes. SQL coordinates GC but never deletes Storage objects. |
