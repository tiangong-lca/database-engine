-- Rework LCA projection RPC service authorization.
--
-- The projection RPCs are already EXECUTE-granted only to service_role. The
-- previous definitions also called util.is_service_request(), which depends on
-- request-header/JWT GUCs that are not guaranteed to be populated for all
-- service-role Supabase client calls. That made deployed Edge Functions receive
-- SERVICE_ROLE_REQUIRED from service-only RPCs. Keep authorization at the grant
-- boundary and remove the duplicate runtime header check.
...
comment on function public.lca_read_job_projection(uuid, uuid, uuid, boolean)
  is 'Service-role LCA job/status/result projection backed by worker_jobs and retained result metadata, not public.lca_jobs. Authorization is enforced by EXECUTE grants.';
comment on function public.lca_read_result_projection(uuid, uuid, text, boolean)
  is 'Service-role LCA result projection with worker_jobs ownership, replacing Edge reads through public.lca_jobs. Authorization is enforced by EXECUTE grants.';
comment on function public.lca_read_latest_single_solve_result(uuid, uuid, integer)
  is 'Service-role latest solve_one result projection from worker_jobs-backed lca_result_cache, replacing lca_jobs payload scans. Authorization is enforced by EXECUTE grants.';
