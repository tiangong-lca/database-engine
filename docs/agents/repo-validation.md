...
lastReviewedNote: "Reviewed the guarded support publication proof after adding independent review-admin approval and owner-scoped database-backed verification."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| guarded dataset maintenance publish RPC | `supabase db reset`; run `supabase/tests/20260711_guarded_dataset_publish.sql` and the legacy `supabase/tests/20260404_dataset_command_rpcs.sql` | include owner-scoped lock/non-disclosure, draft-state rejection, stale `modified_at` and `json_ordered` preconditions, authenticated-only execute grants, independent system-team `review-admin` approval, owner self-approval rejection, verified reviewer email, raw audit-write denial, approval-role revalidation, indexed approval/publish correlation bound to exact table/id/version/plan/action/timestamp/payload, lossless text audit ids in requests/responses/persisted payloads, same-action replay, independently published row rejection, rollback after a forced publish-audit insert failure, and owner-scoped `qry_dataset_publish_guarded_proof` rejection of forged local proof | The guarded RPCs are limited to `unitgroups` and `flowproperties`. Approval and publication are separate authenticated actions. Publication must bind the exact immutable approval audit id and expected reviewer identity before mutation; the owner update and publication audit insert remain one transaction. Verify must use the read-only database proof query instead of trusting local progress or commit artifacts. |
