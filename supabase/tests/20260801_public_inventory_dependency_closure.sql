...
select is((select count(*)::bigint from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relkind in ('r','p')), 46::bigint, 'public physical table inventory is exact after Worker, LCA snapshot-family, document-evidence, and snapshot-GC moves');
select is((select count(*)::bigint from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relkind='v'), 15::bigint, 'public view inventory includes Worker, LCA snapshot-family, document-evidence, and snapshot-GC compatibility views');
