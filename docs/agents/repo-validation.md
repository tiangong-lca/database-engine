...
lastReviewedNote: "Updated for Issue #446: rejected Reference Review re-entry must rebind every eligible active Root Review while preserving finalized roots and append-only history."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Root/Reference Review v2 schema, commands, queue projections, or legacy migration | `supabase db reset`; `supabase test db --local supabase/tests/20260731_root_reference_review_v2.sql`; when rejected-revision resubmission or root rebinding changes, also run `supabase/tests/20260809_rejected_reference_resubmission.sql` and `supabase/tests/20260810_indirect_rejected_reference_root_rebind.sql` | rehearse `root_reference_review_v2_local.sh backup`, restore verification, second-local-copy verification, dry-run, one-row apply, and reconciliation against a disposable production-shaped copy before any hosted cutover | Process alone keeps the numeric Gate. Prove the seven-table state and immutability transitions, active/approved exact-checksum reuse, unchanged rejected-revision re-entry as a new Reference Review, shared rebinding across every eligible active Root Review, finalized-root exclusion, historical rejection preservation, append-only scope history, owner-only notifications, advisory Reviewer outcomes, Admin final decisions, and preservation of legacy `review_id`/`reviews` projections. Never use the backup runner against production until its local restore markers represent real checks. |
