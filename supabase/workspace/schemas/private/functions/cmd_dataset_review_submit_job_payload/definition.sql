CREATE OR REPLACE FUNCTION "private"."cmd_dataset_review_submit_job_payload"("p_job" "anyelement") RETURNS "jsonb"
    LANGUAGE "sql" STABLE
  ...
  select jsonb_strip_nulls(
    jsonb_build_object(
      ...
      'submitWorkerJob',
        (
          select private.worker_job_payload(w, false)
          from private.worker_jobs as w
          where w.id = nullif(row_json->>'submit_worker_job_id', '')::uuid
        ),
      'gate',
        (
          select private.cmd_dataset_review_submit_gate_payload(g)
          from private.dataset_review_submit_gate_runs as g
...
ALTER FUNCTION "private"."cmd_dataset_review_submit_job_payload"("p_job" "anyelement") OWNER TO "postgres";

REVOKE ALL ON FUNCTION "private"."cmd_dataset_review_submit_job_payload"("p_job" "anyelement") FROM PUBLIC;

GRANT ALL ON FUNCTION "private"."cmd_dataset_review_submit_job_payload"("p_job" "anyelement") TO "service_role";

GRANT ALL ON FUNCTION "private"."cmd_dataset_review_submit_job_payload"("p_job" "anyelement") TO "api_internal_executor";
